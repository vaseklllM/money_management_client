...
export default function InputActiveIcon(props: Props): ReactElement {
  ...
  function getIcon() {
    if (isActiveToken === status.loading) return <LoadingOutlined />
    else if (isActiveToken === status.no_data || isActiveToken === status.error) {
      return <ErrIcon />
    } else if (isActiveToken === status.successful) return <SuccessfulIcon />
  }
